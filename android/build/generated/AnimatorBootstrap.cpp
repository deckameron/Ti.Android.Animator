...
#define TAG "ti.android.animator"

using namespace v8;

static Persistent<Object> bindingCache;

static void Animator_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Animator.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::AnimatorBindings::lookupGeneratedInit(
...
static void Animator_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Animator_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Animator_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::AnimatorBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AnimatorBinding = {
	"ti.android.animator",
	Animator_init,
	Animator_dispose
};
...
Java_ti_android_animator_AnimatorBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.android.animator", &AnimatorBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::AnimatorBindings::lookupGeneratedInit));
}
